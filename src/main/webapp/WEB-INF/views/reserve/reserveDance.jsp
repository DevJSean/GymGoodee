...
<script>
	...
	function fngetClasses(){
		$.ajax({
			...
			type: 'get',
			data : 'subject=DANCE&classDate=${classDate}',
			dataType:'json',
				...
				// 2-2-2) 수강권이 있는 경우 아래와 같이
					...
					if(obj.classCount == 0){
						...
						if(memberId == 'admin'){
							...
							if(obj.state == 0){
								$('#table_caption').html('잔여 횟수가 0회입니다.<br><a id="btnPayDance">수강권 구매하기</a>');	
								$('body').on('click','#btnPayDance',function(){
									window.opener.location.href='${contextPath}/pay/payDance';	// 부모창에서 새로운 경로로 이동
									window.close();
								})
							} 
						}
					...
					// 강좌 목록 표시하기
					$.each(obj.classes, function(i,danceclass){
						var tr = $('<tr>');
						tr.append($('<td>').text(danceclass.teacherName));
						tr.append($('<td>').text(danceclass.classTime));
						tr.append($('<td>').text(danceclass.locationCode));
						tr.append($('<td>').text(danceclass.currentCount + "/" + danceclass.locationLimit));
						// 2-1) 관리자의 경우 => [예약 관리] 버튼
						if(memberId == 'admin'){
							tr.append($('<td>').html('<input type="button" class ="btnAdminReservation" data-classcode="'+ danceclass.classCode + '" value="예약관리">'));
						}
						// 2-2) 고객의 경우
						else{
							...
							if(today > danceclass.classDate){
								if(danceclass.reservationState == 1){
							...
							// 당일
							else if(today == danceclass.classDate){
								if(danceclass.reservationState == 0){
									// (1) 내가 예약한 수업이 당일 수업인 경우 [예약 확정] 버튼
									//tr.append($('<td>').html('<input type="button" class ="btnReserveConfirm" data-classcode="'+ danceclass.classCode + '" value="예약확정">'));																		
									tr.append($('<td class ="btnReserveConfirm">').text('예약확정'));	
								} 
								else if(danceclass.reservationState == 1){
									// (2) 수강이 끝난 후
									tr.append($('<td>').html('<input type="button" class ="btnClassEnd" value="리뷰작성">'));
								}
								else if(danceclass.reservationState == -1 || danceclass.reservationState == 500){
									// (2) 당일 강좌 중 내가 예약하지 않은 경우
									var classDateTime = danceclass.classDate + (danceclass.classTime).replace(/:/gi, "");
									...
									if(nowDateTime < classDateTime){
										...
										else if(danceclass.currentCount == danceclass.locationLimit){
	...
	function fnReserve(){
		...
		$('body').on('click','.btnReserve',function(){
			...
			$.ajax({
				...
				type: 'POST',
				data : 'subject=DANCE&classCode=' + classCode,
				dataType: 'json',
				success : function(obj){
					//console.log('버튼 ',obj);
					if(obj.state == 501){
